...
//
/////////////////////////////////////////
function formatArrayEntries(arr){
  let accumulator='';
  for(let i=0;i<arr.length;i++){
    accumulator += `${i+1}. ${arr[i]} `
  }
  return accumulator;
}
...
lineReader.on('line', function (line) {
  ...
  // ORIGINAL: console.log(entry);
  console.log(`${entry.hw}\t${entry.pos}\t${formatArrayEntries(entry.es)}\t${formatArrayEntries(entry.en)}\t${entry.preterit}`);
});
